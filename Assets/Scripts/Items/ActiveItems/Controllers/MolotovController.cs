...
public class MolotovController : ActiveItemController
{
    ...
    {
        GameObject spawnedMolotov = Instantiate(projectilePrefab, transform.position, projectilePrefab.transform.rotation);
        Spawn(spawnedMolotov, Owner);
        spawnedMolotov.GetComponent<MolotovBehaviour>().SetProjectileRpc(closestEnemyPosition, activeItem);
    }
}
